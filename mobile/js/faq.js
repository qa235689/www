$(document).ready(function () {
    ...
    //$('.content_area .article:first').find('.a').slideDown(100);
    //$('.content_area .article:first').find('span').html('<i class="fas fa-chevron-up"></i>')
	
	$('.content_area .article .trigger').click(function(e){  //각각의 질문을 클릭하면
        e.preventDefault();   //<a href="#"> 태그 링크처리
        var myArticle = $(this).parents('.article'); //클릭한 해당 list
	
	if(myArticle.hasClass('hide')){   //클릭한 해당 리스트의 상태가 hide냐? 답변이 닫혀있나?
	    article.find('.a').slideUp(100); //모든 답변 닫아라
        article.removeClass('show').addClass('hide'); //모든 리스트를 hide로 교체
        article.find('span').html('<i class="fas fa-chevron-down"></i>');
